...
### Session: 2026-01-09 - Gini Coefficient Implementation (Issue #19 Fix)
...
### Bernie + Pelosi Test Case (Issue #19 & #20)

**Data Collection with Gini Coefficient:**
- [ ] Bernie Sanders: Collecting - 30,000/37,612 transactions (79.8% done)
  - Reset to collect with Gini coefficient (standard inequality measure)
  - Auto-running every 2 minutes via cron
  - Estimated completion: ~30 minutes remaining
  - Previous: 31,612 txns, 11,419 donors, 2.9% top-10, HHI 0.001465 (incorrect thresholds)
- [ ] Nancy Pelosi: Collecting - 11,998/19,659 transactions (61.0% done)
  - Collecting with Gini coefficient (will resume after Bernie completes)
  - Auto-running every 2 minutes via cron
  - Estimated completion: ~30 minutes after Bernie finishes

**Integration:**
- [x] Gini coefficient calculation implemented in free-tier worker
- [x] Tier calculation updated to use Gini multiplier (replaced HHI)
- [x] Both workers deployed with Gini integration
- [ ] Wait for data collection to complete
- [ ] Test tier recalculation with actual Gini values
- [ ] Validate Bernie (broad base) gets lower penalty than Pelosi (concentrated)

**Analysis (Post-Collection):**
- [ ] Compare Bernie vs Pelosi Gini coefficients
- [ ] Verify Gini-based multiplier logic correctly rewards/penalizes
- [ ] **Expected Result**: Bernie S tier (broad base, low Gini penalty), Pelosi "obliterated" (concentrated donors, high Gini penalty)
- [ ] Document: Issue #19 FIXED - tier calc now accounts for donor distribution
- [ ] Prove: Bernie (low Gini) gets 0.25-0.5× vs Pelosi (higher Gini) gets 1.0-1.5×

**CRITICAL: D1 Migration Required Before Scaling**

**Problem with Current Pure-KV Approach:**
- ❌ Cannot iterate on Gini thresholds without re-collecting (90+ minutes per iteration)
- ❌ Cannot try alternative metrics (Theil index, different percentiles) without re-collection
- ❌ Already wasted 4+ hours re-collecting same data after changing HHI → Gini
- ❌ Deleted Bernie analysis data and started over (lost 2 hours of collection)
- ❌ Stream-and-aggregate optimizes for wrong constraint (storage we don't need to save)

**Solution: Dual Storage Architecture (KV + D1):**

1. **Set Up D1 Database**:
   ```bash
   wrangler d1 create taskforce-purple-donors
   # Add binding to wrangler-free-tier.toml
   ```
...
2. **Create Schema**:
   ...
   CREATE TABLE itemized_transactions (
     ...
     receipt_date TEXT,
     INDEX idx_bioguide (bioguide_id)
   );
   ...
   CREATE TABLE donor_aggregates (
     ...
     PRIMARY KEY (bioguide_id, donor_key)
   );
   ```

3. **Modify Worker to Write Both**:
   - D1: Raw transactions for analytical queries
   - KV: Final metrics cache for fast API lookups
   - Storage: ~6 MB for 20 high-profile members (well under 5 GB D1 free tier)

4. **Benefits**:
   - ✅ Calculate ANY metric instantly via SQL (2 seconds vs 90 minutes)
   - ✅ Iterate on Gini thresholds without re-collection
   - ✅ Try alternative inequality measures (Theil, Atkinson, percentile ratios)
   - ✅ Adjust concentration penalty formula without touching FEC API
   - ✅ Query donor distributions for debugging/analysis

**Next Steps (After Bernie/Pelosi Validation):**
- [ ] If Gini calculation works (Bernie S tier, Pelosi drops): Migrate to D1 before scaling
- [ ] Set up D1 database and schema
- [ ] Modify worker to dual-write (D1 + KV)
- [ ] Re-collect Bernie + Pelosi into D1 (last time!)
- [ ] Validate can recalculate metrics from SQL queries
- [ ] Then scale to remaining ~533 members with confidence

**Do NOT scale to 535 members on pure-KV** - will create maintenance nightmare for metric iteration.
